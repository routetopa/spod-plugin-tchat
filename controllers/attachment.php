 ...
 * @since 1.0
 */
class SPODTCHAT_CTRL_Attachment extends OW_ActionController
{
    /**
        ...
        if ( OW::getUser()->isAuthenticated() && !empty($_POST['flData']) && !empty($_POST['pluginKey']) && !empty($_FILES['ow_file_attachment']) )
        {
            ...
            foreach ( $finalFileArr as $item )
            {
                try
                {
                    ...
                    $respondArr['url'] = $dtoArr['url'];
                    $respondArr['type'] = (in_array(explode('.', $_FILES['ow_file_attachment']['name'][0])[1], array('jpg', 'jpeg', 'gif', 'png'))) ? 'image' : 'file';
                }
                catch ( Exception $e )
                {
                ...
                {
                    $respondArr['items'][$nameArray[$item['name']]] = array('result' => true, 'dbId' => $dtoArr['dto']->getId(), 'url' => $respondArr['url'], 'type' => $respondArr['type']);
                }
            }

            $items = BOL_AttachmentService::getInstance()->getFilesByBundleName($pluginKey, $bundle);
